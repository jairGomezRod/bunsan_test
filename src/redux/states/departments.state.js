...
export const departmentSlice = createSlice({
  ...
  reducers: {
    ...
    deleteEmployeesDepartment: (state, action) => {
      let pos = state.map(item => item.id).indexOf(action.payload.dad);
      state[pos].allocation = state[pos].allocation - action.payload.expense.allocation;
      switch(action.payload.type){
        case 1:
          state[pos]["eManager"] --;
...
export const { newDepartment, updateEmployeesDepartment, deleteEmployeesDepartment } = departmentSlice.actions
